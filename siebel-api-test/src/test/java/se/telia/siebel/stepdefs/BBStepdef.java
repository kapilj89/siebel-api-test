...
public class BBStepdef implements En {
    DataStorage dataStorage;
    public BBStepdef( DataStorage dataStorage) {
...
//            String AddressId = dataStorage.getPrimaryAddressId();
...
//            System.out.println("Adding copper max HD & SD");
        ...
        And("^call AddServiceAdress format StreetName,StreetNumber,Entrance,City,Apartnumber,PointID;RowID;Postalcode \"([^\"]*)\"$", (String AddressData) -> {
            System.out.println("\nQuerySelfServiceAccount\n");
            Map<String,String> AddressMap= new HashMap<>();
            String[] AddressParam=AddressData.split(";");
            AddressMap.put("StreetAddress", AddressParam[0]);
            ...
            AddressMap.put("City", AddressParam[3]);
            AddressMap.put("ApartmentNum", AddressParam[4]);
            AddressMap.put("PostalCode", AddressParam[7]);
            AddressMap.put("PointId", AddressParam[5]);
            AddressMap.put("RowID", AddressParam[6]);
        ...
        And("^call BBVOIPIPTV SynchronizeQuote to populate the deliveryContract \"([^\"]*)\" Revision \"([^\"]*)\" RowID \"([^\"]*)\" and AccessCode \"([^\"]*)\" on the quote line items for a MDU order$", (String Agreement,String RevisionNumber,String RowID,String AccessCode) -> {
            ...
            quoteItemList.stream()
                    ...
                    .forEach(quoteItem -> {
                        ...
                        if(quoteItem.getLineNumber().equals("2")|| quoteItem.getLineNumber().equals("3")|| quoteItem.getLineNumber().equals("7")|| quoteItem.getLineNumber().equals("8")|| quoteItem.getLineNumber().equals("9")|| quoteItem.getLineNumber().equals("11")){
//                    	   quoteItem.setTSDeliveryContractId("1-1658X79");1-16891A1
        ...
        And("^call SDU BBIPTV_VOIP SynchronizeQuote to populate the AccessCode \"([^\"]*)\" on the quote line items for a SDU order$", (String accessType) -> {
            ...
            quoteItemList.stream()
                    ...
                    .forEach(quoteItem -> {
                        quoteItem.setTSC2BDueDate(dueDate);
                        quoteItem.setRequestedDeliveryDate(dueDate);            
                       if(quoteItem.getLineNumber().equals("2")|| quoteItem.getLineNumber().equals("3")|| quoteItem.getLineNumber().equals("7")|| quoteItem.getLineNumber().equals("9")|| quoteItem.getLineNumber().equals("8")|| quoteItem.getLineNumber().equals("11")){
        ...
        And("^call AddServiceAdress for XDSL format StreetName,StreetNumber,Entrance,City,Apartnumber,PointID;Postalcode \"([^\"]*)\"$", (String AddressData) -> {
            System.out.println("\nQuerySelfServiceAccount\n");
            Map<String,String> AddressMap= new HashMap<>();
            String[] AddressParam=AddressData.split(";");
            AddressMap.put("StreetAddress", AddressParam[0]);
...
//          Map<String,String> AddressMap =datatable.asMap(String.class,String.class);
//            
//            for(String Address : AddressMap.keySet()){
//                System.out.println(Address +" : "+AddressMap.get(Address));
//            }           
            QuerySelfServiceAccountExecute querySelfServiceAccount = new QuerySelfServiceAccountExecute(dataStorage);
            querySelfServiceAccount.SetUpdateAddress(AddressMap, "2");
        });
        
        
        
        And("^call AddressAddorupdate with format StreetName,StreetNumber,Entrance,City,Apartnumber,PointID;Postalcode \"([^\"]*)\"$", (String AddressData) -> {
            System.out.println("\nQuerySelfServiceAccount\n");
            Map<String,String> AddressMap= new HashMap<>();
            String[] AddressParam=AddressData.split(";");
            AddressMap.put("StreetAddress", AddressParam[0]);
...
//            AddressMap.put("RowID", AddressParam[6]);
        ...
        And("^call AddServiceAdress format StreetName,StreetNumber,Entrance,City,Apartnumber,PointID;Postalcode \"([^\"]*)\"$", (String AddressData) -> {
            System.out.println("\nQuerySelfServiceAccount\n");
            Map<String,String> AddressMap= new HashMap<>();
            String[] AddressParam=AddressData.split(";");
            AddressMap.put("StreetAddress", AddressParam[0]);
...
//            }           
        ...
        And("^call QuerySelfServiceAddress Add/Update for XDSL format StreetName,StreetNumber,Entrance,City,Apartnumber,PointID;Postalcode \"([^\"]*)\"$", (String AddressData) -> {
...
//                    	   quoteItem.setTSDeliveryContractId("1-1658X79");1-16891A1
        ...
        And("^call QuerySelfServiceAddress \"([^\"]*)\"$", (String AddressData) -> {
            System.out.println("\nQuerySelfServiceAccount\n");

            List<String> Addresslist = new ArrayList<String>(Arrays.asList(AddressData.split(";")));                     		
...
//            System.out.println("ProductID is :"+productID +".  Integration ID is :"+IntergrationID);
...
//							else if(relation.getName().equalsIgnoreCase(PackageName)){
//								System.out.println("Package:" + relation.getName());
...
//            Assert.assertEquals("commitmentDuration is wrong value after end configuration",commitmentDuration, dataStorage.getCommitmentDuration());
        ...
        And("^call SynchronizeQuote to populate the AccessCode \"([^\"]*)\" on the quote line items for a SDU order$", (String accessType) -> {
            System.out.println("\nSynchronizeQuote for BroadBand\n");
            Quote quote = dataStorage.getQuote();
//            String dueDate = SiebelDateFormat.siebelDateFormat(SiebelDateFormat.getCurentCETtime()).trim();
            String dueDate;
			try {
        ...
        And("^call xDSLSynchronizeQuote to populate the AccessCode \"([^\"]*)\" and ConnRef number \"([^\"]*)\" on the quote line items for a xDSL order$", (String accessType, String fbNumber) -> {
            System.out.println("\nSynchronizeQuote for BroadBand\n");
            Quote quote = dataStorage.getQuote();            
            String dueDate = SiebelDateFormat.siebelDateFormat(SiebelDateFormat.getTomorrowsDate());
            String dueDateHardware = SiebelDateFormat.siebelDateFormat( new Date());
            ...
            quoteItemList.stream()
                    ...
                    .forEach(quoteItem -> {
                        quoteItem.setTSC2BDueDate(dueDate);
                        quoteItem.setRequestedDeliveryDate(dueDate);            
                       if(quoteItem.getLineNumber().equals("2")|| quoteItem.getLineNumber().equals("3")||quoteItem.getLineNumber().equals("10")){
                    	   quoteItem.setTSAccessTypeId(accessType);
        ...
        And("^call xDSL DataPrep SynchronizeQuote to populate the AccessCode \"([^\"]*)\" PSTN \"([^\"]*)\" and ConnRef number \"([^\"]*)\" on the quote line items for a xDSL order$", (String AccessCode, String PSTN, String FbNumber) -> {
            System.out.println("\nSynchronizeQuote for BroadBand\n");
            Quote quote = dataStorage.getQuote();            
            String dueDate = SiebelDateFormat.siebelDateFormat(SiebelDateFormat.getTomorrowsDate());
            String dueDateHardware = SiebelDateFormat.siebelDateFormat( new Date());
            ...
            quoteItemList.stream()
                    ...
                    .forEach(quoteItem -> {
                        quoteItem.setTSC2BDueDate(dueDate);
                        quoteItem.setRequestedDeliveryDate(dueDate);            
                       if(quoteItem.getLineNumber().equals("2")|| quoteItem.getLineNumber().equals("3")||quoteItem.getLineNumber().equals("7")||quoteItem.getLineNumber().equals("9")){
                    	   quoteItem.setTSAccessTypeId(AccessCode);
                    	   quoteItem.setConnectivityReferenceNumber(FbNumber);
        ...
        And("^call SynchronizeQuote to populate the deliveryContract \"([^\"]*)\" on the quote line items for a MDU order$", (String Agreement) -> {
            System.out.println("\nSynchronizeQuote for BroadBand\n");
            Quote quote = dataStorage.getQuote();
            String dueDate = SiebelDateFormat.siebelDateFormat(SiebelDateFormat.getTomorrowsDate());
            String dueDateHardware = SiebelDateFormat.siebelDateFormat( new Date());
...
//                    	   quoteItem.setTSDeliveryContractId(Agreement);
        ...
        Then("^call SubmitOrder Service and Get successful OrderID", () -> {
            ...
            System.out.println(dataStorage.getOrder());
        });
        
        And("^call Addproduct to set RelationShipID \"([^\"]*)\" and ProductID \"([^\"]*)\"", (String RelationShipID, String ProductID ) -> {
            System.out.println("Inside Add product");
             List<Item> itemList =  dataStorage.getListOfData().getProductData().getItem();
             for (Item item: itemList) {
                 String innerItemList = item.getIntegrationId();
